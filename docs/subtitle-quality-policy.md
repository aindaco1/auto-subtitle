...
Research checked September 7, 2026. Policy target: `subtitle-quality-v1`. This document owns subtitle readability, cleanup, and format-validation rules for the [implementation plan](implementation-plan.md). Safe wrapping, exact continuous-duplicate merging and readability warnings are implemented in `engine/quality.mjs`; generated grouping uses shared timed-text. Detailed language profiles and imported cue subdivision remain targets rather than completed behavior. The [September 23 evaluation](jev-apple-formatting.md) adds bounded on-device Apple formatting, developer-only Jev evaluation, and fixes duplicate idempotence and soft-wrap reading-speed accounting.

**Scope and defaults**

Every Align, Generate, and Improve export uses one shared quality pass. **Clean up formatting** defaults on for imported subtitles, including translations, and can be turned off under Options for exact cue/text-layout preservation. Generate uses the same cleanup switch; baseline recognition grouping still uses observed word boundaries. This is independent of **Improve accuracy**, which authorizes same-language wording corrections. Implemented cleanup rewraps text and consolidates redundant identical captions without translating or paraphrasing. Imported subdivision and display-time extensions below remain policy targets. Apple-assisted layout is limited to English and Spanish on supported systems. Punctuation/capitalization runs with cleanup and requires review; audio-supported spelling remains with Improve.
