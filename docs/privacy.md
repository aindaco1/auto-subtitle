...
Audio, video, subtitles, speech activity, recognized words, and change audits stay on the Mac. Apple formatting uses the on-device Foundation Models API; it does not send captions to Jev or a cloud model. Network access occurs for GitHub update checks/downloads, explicit model setup, explicit report submission, or a user-opened source/issue link. Sparkle checks the official release feed quietly at launch; installation requires user action. Its system profiling and automatic installation are disabled.
...
Developer-only Jev runs require an explicit live command and accept only the allowlisted public synthetic corpus. Credentials and private subtitle paths are never accepted as fixture input. See [evaluation](jev-apple-formatting.md).
