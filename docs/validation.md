...
- Relay source was validated with 32 tests and deployment dry-run, then deployed as Worker `f603dbf7-0e92-4e6e-8887-b5ecb45c9e30`. The canonical client/server contract is byte-identical.

Notarization, hosted CI, public-download and installed-update acceptance are recorded below when complete. Private detailed release evidence stays under ignored `artifacts/releases/1.0.0/`.
...
4. Clean macOS 15 on an 8 GB Apple Silicon Mac, peak memory/disk/time, network-denied whole-app operation, Developer ID/notarization, and external installed-artifact acceptance. The local DMG mounted, its signature verified, and its installed copy passed the local runtime/import/generation/save checks.
