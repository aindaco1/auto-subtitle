...
import { flat, sameWords, eligible, preservesPunctuation, preservesWordCase } from '../../engine/formatting-policy.mjs';
...
export function literalFailures(fixture,cues) {
  const text=cues.map(c=>visibleText(c.text,fixture.format)), rules=fixture.literal??{}, failures=[];
  for(const phrase of rules.sameLine??[]) if(!text.some(c=>c.split('\n').some(line=>line.includes(phrase)))) failures.push(`Phrase split across lines: ${phrase}`);
  for(const index of rules.continuations??[]) if(!text[index]||/[.!?…]["'”’)]*$/.test(text[index].trim())) failures.push(`Caption ${index+1} closes a continuing sentence`);
  for(const {cue,mark} of rules.endings??[]) if(!text[cue]?.trim().endsWith(mark)) failures.push(`Caption ${cue+1} must end with ${mark}`);
  for(const {cue,mark} of rules.beginnings??[]) if(!text[cue]?.trim().startsWith(mark)) failures.push(`Caption ${cue+1} must start with ${mark}`);
  for(const cue of rules.capitalized??[]) if(!/^\P{L}*\p{Lu}/u.test(text[cue]??'')) failures.push(`Caption ${cue+1} needs initial capitalization`);
  return failures;
}
export function evaluateOutput(fixture,cues) {
  ...
  for(let i=0;i<cues.length;i++) {
    ...
    if(fixture.punctuation&&(!preservesPunctuation(source,text)||!preservesWordCase(source,text))) failures.push('Existing punctuation or internal capitalization changed');
  ...
  return {failures:[...new Set(failures)],formattingFailures:literalFailures(fixture,cues),warnings:quality.warnings};
}
export function prepareJev(report) {
  ...
  return fixtures.map((fixture,i)=>{
    ...
    Object.assign(saved,check); // Recompute literal rules; never trust saved pass/fail fields.
...
export function summarize(report,jev) {
  ...
  const formatting=report.cases.every(c=>(c.formattingFailures??[]).length===0);
  ...
  const complete=jev?.complete&&jev.cases.length===fixtures.length&&fixtures.every((fixture,i)=>{
    const row=jev.cases[i];
    return row.id===fixture.id&&Object.keys(fixture.requirements).every(key=>row.result?.findings?.[key]);
  });
  return {deterministic,formatting,native,semantic:!jev?'not-run':!complete?'incomplete':findings.some(f=>f.decision==='fail')?'fail':findings.some(f=>f.decision==='review')?'review':'pass',
    passed:deterministic&&formatting&&native!=='incomplete'&&!!complete&&findings.length>0&&findings.every(f=>f.decision==='pass'),releaseAccepted:false};
}
