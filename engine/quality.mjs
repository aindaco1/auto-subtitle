...
export const protectedText = (text, format) => format === 'ass' ? /\{[^}]*\}|\\[nh]/.test(text) : /<[^>]*>/.test(text);
export function wrapOptions(text, limit = 42, maximumOptions = 12) {
  ...
  for (const i of spaces) {
    ...
    choices.push({ index:i, cost });
  }
  return choices.length ? choices.sort((a,b) => a.cost-b.cost).slice(0,maximumOptions).map(c => flat.slice(0,c.index)+'\n'+flat.slice(c.index+1)) : [text];
}
export const wrap = (text, limit = 42) => wrapOptions(text, limit, 1)[0];
...
export function cleanCues(input, { format='srt', language='auto', cleanup=true, generated=false, durationMs=Infinity, fps=0 }={}) {
  ...
  const comparison = text => text.replace(format === 'ass' ? /\\N/g : /\n/g, ' ');
  for (const original of input) {
    ...
    if (cleanup && continuous && plain && comparison(c.text) === comparison(prev.text) && c.style === prev.style) {
