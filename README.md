...
- Cleanup preserves already capitalized, punctuated captions and existing punctuation. Short continuous phrases share a bounded punctuation pass, then map back to their original words and timings. It also polishes punctuation and capitalization automatically when Apple Intelligence is available. It preserves words and timing, including translated captions. Review changes in the Local report before saving.
- Generate uses local Parakeet v3 through Platform’s shared DustWaveSpeech. Passage-level checks flag unexpected language changes. Optional local Whisper retries repair only wording supported by two audio decodes; uncertain passages remain unchanged and are listed for review.
